...
Canonical .emu path resolution for macOS / Linux.
...
def get_emu_path_str() -> str:
    """Return `.emu` as a POSIX string.

    Use this when you need to embed the path in a prompt, log message,
    or any string destined for the Electron frontend or shell.
    """
    return str(get_emu_path())


def get_project_root_str() -> str:
    """Return the project root as a POSIX string."""
