"""
client_compact.py — H Company (Holo) compaction client

Uses the same model/endpoint to summarise bloated context chains.
"""
...
def compact(messages: list["PreviousMessage"]) -> str:
    ...
    print(f"[compact/h_company] Summarised in {elapsed_ms}ms  ({len(summary)} chars)")
    return summary
