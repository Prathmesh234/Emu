"""
client.py — Together AI API client

Uses the OpenAI Chat Completions API format against Together's endpoint.

Docs: https://docs.together.ai/reference/chat-completions

Environment:
    TOGETHER_API_KEY  — required
    TOGETHER_MODEL    — optional (default: meta-llama/Llama-4-Maverick-17B-128E-Instruct-FP8)
"""
...
def ensure_ready(**kwargs) -> None:
    if not API_KEY:
        raise RuntimeError(
            "[together_ai] TOGETHER_API_KEY is not set. "
...
def _parse_response(resp, elapsed_ms: int) -> AgentResponse:
    ...
    if message and message.tool_calls:
        ...
        print(f"[together_ai] tool_calls: {[tc.name for tc in tool_calls]}")
    ...
    return AgentResponse(
        action=safe_build_action(raw_action, "together_ai"),
...
def _sanitize_single_action(data: dict) -> dict:
    for key in ("next", "next_action", "actions", "step2", "then"):
        if key in data:
            print(f"[together_ai] WARN: stripped batched key '{key}' from response")
            del data[key]
    return data
...
def _extract_json(content: str) -> dict:
    ...
    print(f"[together_ai] INFO: plain-text response, wrapping as unknown:\n  {content[:200]}")
