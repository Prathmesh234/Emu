...
COWORKER_DRIVER_TOOLS_OPENAI: list[dict] = [
    ...
    _fn(
        ...
        (
            ...
            "When the target window is on another macOS Space, the driver "
            "keeps background mode safe by skipping the AX walk for that "
            "call and returning a vision-only screenshot instead; do not "
            "expect fresh tree_markdown or element_index values, and do not "
            "reuse stale indices from an earlier snapshot. Prefer "
            "element_index for actions only when this response includes a "
            "fresh AX tree. If you must use pixels, read "
            ...
            "When AX is walked, this mints a fresh index map and invalidates "
            "the previous one for this (pid, window_id). INVARIANT: before "
            "any element-indexed action, use the latest response that actually "
            "included an AX tree for that exact (pid, window_id). window_id MUST belong to pid. Optional "
            ...
            "Allow-JavaScript-from-Apple-Events) and can still return when "
            "the AX walk was skipped. Capture mode (som / "
            "vision / ax; default som) is set persistently via "
            "cua_set_config. If a snapshot lacks tree_markdown, read the tool "
            "text first: an off-Space warning means AX was intentionally skipped; "
            "otherwise check cua_get_config before assuming the app exposes no AX tree."
        ),
        {
            "pid": _PID,
