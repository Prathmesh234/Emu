...
async def handle_invoke_hermes(
    ...
    return (
        ...
        "DEFAULT BEHAVIOUR: end the current turn now. Do NOT call check_hermes "
        "in this turn. Send a final_message to the user along the lines of: "
        "'I've delegated this to the Hermes agent — it's running in the "
        "background. Let me know when you'd like me to check in on it.' Then "
        "emit a `done` action and yield control back to the user.\n\n"
        "Only deviate from this if the user explicitly asked you to wait for "
        "Hermes to finish before responding. In that case, call "
        f"`check_hermes(job_id='{job.job_id}', wait_s=60)` and loop.\n\n"
        "When the user later asks for an update (e.g. 'check on hermes', "
        "'is it done yet?'), call "
        f"`check_hermes('{job.job_id}')`. To abort the job, call "
