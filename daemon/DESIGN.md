...
We therefore enforce **defense in depth**, four layers deep:
...
| L3: OS sandbox | Process launched under `sandbox-exec` with a custom `.sb` profile that allows file writes *only* on `.emu/` plus the daemon's own code dir read-only, and network egress only on 443. | Bugs in L1/L2, library side-effects, subprocess spawn |
| L4: launchd hygiene | Runs as the logged-in user (never root). `Nice = 5`, hard CPU ceiling, single-instance via `flock`, logs to `.emu/global/daemon/logs/`. | Runaway resource usage, concurrent runs corrupting files |
...
```
┌───────────────────────────────────────────────────────────────────────┐
│ launchd  (user agent, ~/Library/LaunchAgents/com.emu.memory-daemon.plist)│
│   plist is generated by the Emu app on first launch, templated with     │
│   the user's resolved $HOME and .emu path.                              │
│   StartInterval = 300   (every 5 minutes)                               │
└─────────────────────────────┬─────────────────────────────────────────┘
                              │ exec
                              ▼
┌───────────────────────────────────────────────────────────────────────┐
│ /usr/bin/sandbox-exec -f <app bundle>/Resources/daemon/emu-daemon.sb  │
│   <app bundle>/Resources/daemon/python -m daemon.run                  │
└─────────────────────────────┬─────────────────────────────────────────┘
                              │
                              ▼
┌───────────────────────────────────────────────────────────────────────┐
│ daemon.run (Python)                                                   │
│   1. resolve EMU_ROOT from env var EMU_ROOT, else default to           │
│      $HOME/.emu                                                        │
│   2. acquire flock($EMU_ROOT/global/daemon/.tick.lock)                │
│   3. load processed-session state                                     │
│   4. inventory $EMU_ROOT/sessions/, filter to UNPROCESSED only        │
│      (incremental always — no backfill)                               │
│   5. if nothing new → exit 0                                          │
│   6. build prompt = DAEMON_PROMPT (inline string) + INPUT manifest    │
...
## 4. File Layout (in the repo / app bundle)

```
daemon/
├── DESIGN.md                       ← this document
├── launchd/
│   └── com.emu.memory-daemon.plist.template   ← placeholders for $HOME, $EMU_ROOT, $APP_BUNDLE
├── sandbox/
│   └── emu-daemon.sb.template      ← sandbox-exec profile, same placeholders
├── run.py                          ← entrypoint (`python -m daemon.run`)
...
At app build time, `daemon.md` is embedded as a Python string constant into
`prompt.py` so the runtime never reads it from disk.
...
`stdout.log` / `stderr.log` are launchd's view and are kept small via
in-process log rotation inside `run.py`.
